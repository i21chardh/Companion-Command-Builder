...
function presetDocument() {
  ...
  return { format: 'companion-command-builder-layout', schemaVersion: 1, appVersion: '0.20.84', name: presetFileHandle?.name?.replace(/\.(?:json|ccb-layout)$/i, '') || 'Untitled layout', model: modelSelect.value, savedAt: new Date().toISOString(), pages, workspaceSurfaces };
}
...
async function preview() {
  ...
  try {
    ...
    if (graphic) for (const item of currentPlans) {
      const base = removeAppliedGraphic(item.button.text, item.button.graphic);
