...
const input = {
  ...
  peerA: { name: 'FOH', surfaceId: 'deck-a', call: '1/0/0', alarm: '1/0/1', talkOn: '2/1/0', talkOff: '2/1/1' },
  peerB: { name: 'MON', surfaceId: 'deck-b', call: '1.0.0', alarm: '1.0.1', listenOn: '2/2/0', listenOff: '2/2/1' },
};

test('builds a four-button two-surface Call and Alarm workflow', () => {
  ...
  assert.deepEqual(result.plans.map((plan) => plan.intercom.role), ['call-peer-b', 'alarm-peer-b', 'call-peer-a', 'alarm-peer-a']);
  assert.equal(result.plans[0].button.action.definitions[0].definitionId, 'custom_variable_set_value');
  assert.equal(result.plans[1].button.action.definitions[1].options.location, '2/1/0');
  assert.equal(result.plans[3].button.action.definitions[1].options.location, '2/2/0');
  ...
  assert.match(result.plans[1].button.behavior, /with or without an alert/i);
  assert.match(result.plans[1].button.behavior, /first press opens/i);
  assert.match(result.plans[1].button.behavior, /next press resets/i);
});
...
test('maps intercom plans to Companion internal actions and readable summaries', () => {
  ...
  assert.deepEqual(actionDefinitions(call.button.action).map((item) => item.definitionId), ['custom_variable_set_value']);
...
test('accepts slash and dot coordinates and rejects unsafe workflow collisions', () => {
  ...
  assert.throws(() => buildPeerIntercomPlans({ ...input, peerA: { ...input.peerA, alarm: input.peerA.call } }), /overlap/);
  const oneSurface = buildPeerIntercomPlans({ ...input, peerB: { ...input.peerB, surfaceId: 'deck-a', call: '1/1/0', alarm: '1/1/1' } });
