...
function auditSourceContracts() {
  ...
  const cases = [
    ...
    ['native-save-picker-wired', /fetch\('\/api\/presets\/save'/.test(app) && /path: saveAs \? '' : presetFileHandle\?\.path \|\| ''/.test(app), true],
    ['native-load-picker-wired', /fetch\('\/api\/presets\/load'/.test(app) && /installPreset\(loaded\.document, \{ path: loaded\.path, name: loaded\.name \}\)/.test(app), true],
    ...
    ['updated-module-version-reconfigures-live-schema', /refreshUpdatedConnectionAdapter\(address, connection, onboarding\)/.test(server) && /version: connection\.moduleVersionId, useAi: false, definitions/.test(server) && /!record\?\.compiledAdapter/.test(server), true],
    ['dynamic-rotary-validates-live-schema', /\['dynamic', 'dynamic-rotary'\]\.includes\(plan\.button\?\.action\?\.family\)/.test(server), true],
    ...
    ['lv1-fader-shorthand-bypasses-ai', /input\|channel\|ch\|fader/.test(readFileSync(new URL('../src/deterministic-dynamic.js', import.meta.url), 'utf8')) && /actionId = 'outGain'/.test(readFileSync(new URL('../src/deterministic-dynamic.js', import.meta.url), 'utf8')), true],
    ['lv1-rotary-send-uses-state-aware-action-sets', /sendGainRelative/.test(readFileSync(new URL('../src/deterministic-dynamic.js', import.meta.url), 'utf8')) && /rotate_left/.test(readFileSync(new URL('../src/deterministic-dynamic.js', import.meta.url), 'utf8')) && /rotate_right/.test(readFileSync(new URL('../src/deterministic-dynamic.js', import.meta.url), 'utf8')), true],
  ];
  return cases.map(([id, actual, expected]) => ({
